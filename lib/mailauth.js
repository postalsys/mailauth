...
const { getAlignment } = require('./tools');
...
const authenticate = async (input, opts) => {
    ...
    if (!opts.disableDmarc && dkimResult?.headerFrom) {
        ...
        if (dmarcResult.alignment?.dkim?.strict) {
            // the DKIM verifier only knows relaxed alignment, it runs before the DMARC record
    ...
    if (dkimResult?.seal && (['none', 'pass'].includes(arcResult?.status?.result) || arcResult?.status?.shouldSeal)) {
        ...
        if (sealResult?.errors?.length) {
            // sealing failed, usually an unusable key. The message goes out unsealed, so
            // say why somewhere the caller can find it rather than dropping it silently
