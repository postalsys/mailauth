...
const headerParser = buf => {
    ...
    const parse = () => {
        ...
        for (let i = parts.length - 1; i >= 0; i--) {
            ...
            } else if (['l', 'v', 't'].includes(parts[i].key)) {
        ...
        if (parts[defaultPos].key && !parts[defaultPos].value) {
        ...
        parts.forEach(part => {
            ...
            if (['arc-authentication-results', 'authentication-results'].includes(headerKey) && typeof part.value === 'string') {
                ...
                result[part.key] = Object.assign(result[part.key], valueParser(result[part.key].value));
            }

            if (part.comment) {
                result[part.key].comment = part.comment;
            }
        });
