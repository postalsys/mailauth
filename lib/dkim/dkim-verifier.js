...
const { getSigningHeaderLines, getPublicKey, parseDkimHeaders, formatAuthHeaderRow } = require('../../lib/tools');
...
class DkimVerifier extends MessageParser {
    ...
    async messageHeaders(headers) {
        ...
        try {
            ...
            if (this.arChain.chain?.length) {
                this.arChain.lastEntry = this.arChain.chain[this.arChain.chain.length - 1];
            }
        } catch (err) {
            this.arChain = { err };
        }
        ...
        if (this.arChain.lastEntry) {
            const signatureHeader = this.arChain.lastEntry['arc-message-signature'];
        ...
        try {
            ...
            for (let signatureHeader of this.signatureHeaders) {
                ...
                switch (signatureHeader.type) {
                    case 'ARC':
                        if (this.arChain.lastEntry) {
                            this.arChain.lastEntry.messageSignature = result;
                        }
                        break;
                    case 'DKIM':
                    default:
