...
class DkimVerifier extends MessageParser {
    ...
    async messageHeaders(headers) {
        ...
        for (let signatureHeader of this.signatureHeaders) {
            ...
            signatureHeader.bodyCanon = signatureHeader.canonicalization.split('/').pop().toLowerCase().trim();
            ...
            if (!this.bodyHashes.has(signatureHeader.bodyHashKey)) {
                this.bodyHashes.set(signatureHeader.bodyHashKey, dkimBody(signatureHeader.bodyCanon, signatureHeader.hashAlgo));
            }
        }
    }
    ...
    async finalChunk() {
        ...
        for (let signatureHeader of this.signatureHeaders) {
            ...
            if (signatureHeader.parsed.bh !== bodyHash) {
                ...
                try {
                    publicKey = await getPublicKey(`${signatureHeader.selector}._domainkey.${signatureHeader.signingDomain}`);
                    try {
