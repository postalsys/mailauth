...
class RelaxedHash {
    ...
    _pushBodyHash(chunk) {
        ...
        // Buffer the line endings at the end of the chunk: these are the empty lines
        // that are dropped if no more content follows
        let end = chunk.length;
        while (end > 0 && chunk[end - 1] === CHAR_LF) {
            end -= 1;
            if (end > 0 && chunk[end - 1] === CHAR_CR) {
                end -= 1;
            }
        }
        ...
        if (end < chunk.length) {
            this.emptyLinesQueue.push(chunk.subarray(end));
        }
    ...
    update(chunk, final) {
        ...
        if (final) {
            ...
            if (chunkPart && chunkPart.length) {
                // the last line has no line ending, so its trailing whitespace was
                // never seen by the line break checks above. Always canonicalize it
                this._pushBodyHash(this.fixLineBuffer(chunkPart));
            }
