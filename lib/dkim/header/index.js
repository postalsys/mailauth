...
const generateCanonicalizedHeader = (type, signedHeaderLines, options) => {
    options = options || {};
    ...
    switch (canonicalization) {
        case 'simple':
            return simpleHeaders(type, signedHeaderLines, options);
        case 'relaxed':
            return relaxedHeaders(type, signedHeaderLines, options);
        default:
