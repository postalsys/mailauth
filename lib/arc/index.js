...
const verifyAS = async (chain, opts) => {
    ...
    for (let i = 0; i < chain.length; i++) {
        ...
        if (!isLast) {
            ...
            chunks.push(stripSignatureValue(formatRelaxedLine(link['arc-seal'].original)));
        }
    }
...
const createSeal = async (input, data) => {
    ...
    if (!messageSignature) {
        ...
        return { headers: [], errors: dkimSigner.errors };
    }
    ...
    return {
        headers: [arcSeal, messageSignature, authResults],
