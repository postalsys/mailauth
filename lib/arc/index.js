...
const { parseDkimHeaders, formatRelaxedLine, getPublicKey, formatAuthHeaderRow, formatSignatureHeaderLine, writeToStream } = require('../../lib/tools');
...
const getARChain = headers => {
    ...
    for (let i = 0; i < arcChain.length; i++) {
        ...
        if (arcInstance['arc-seal']?.parsed?.h) {
            let err = new Error(`i=${arcInstance.i} unexpected h`);
