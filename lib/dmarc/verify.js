...
const { formatAuthHeaderRow } = require('../tools');
...
const getAligment = (fromDomain, domainList, strict) => {
    console.log('GA', fromDomain, domainList, strict);
...
const verifyDmarc = async opts => {
    ...
    return formatResponse({ status, domain: orgDomain || domain, policy, p: dmarcRecord.p, sp: dmarcRecord.sp || dmarcRecord.p });
};
