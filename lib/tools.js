...
// RFC 5322 section 2.2.3: a header line is folded by CRLF followed by WSP, and WSP
// is SP or HTAB. The rows are 'binary' strings, so any other byte that a Unicode
// aware check would call whitespace, such as the 0xA0 of a UTF-8 non-breaking space,
// starts a header of its own. formatRelaxedLine reads the lines the same way
...
const formatRelaxedLine = (line, suffix) => {
    let result =
        line
            ...
            .replace(/^([^:]*):[ \t]*/, (m, k) => k.toLowerCase().replace(/^[ \t]+|[ \t]+$/g, '') + ':')
            // single WSP
            .replace(/[ \t]+/g, ' ')
            // no trailing WSP
