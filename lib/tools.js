...
const getPublicKeyWrapped = async rr => {
...
// TODO: fix static  caching
const getPublicKey = async rr => {
    if (dnsCache.has(rr)) {
        //console.log('DNS MATCH:', rr);
        let data = JSON.parse(dnsCache.get(rr));
        if (data.error) {
            let err = new Error(data.error.message);
            Object.keys(data.error).forEach(key => {
                if (key !== 'message') {
                    err[key] = data.error[key];
                }
            });
            throw err;
        }
        return typeof data.pubKey === 'string' ? Buffer.from(data.pubKey) : data.pubKey;
    }

    //console.log('DNS MISS:', rr);
    try {
        let pubKey = await getPublicKeyWrapped(rr);
        dnsCache.set(rr, JSON.stringify({ pubKey: pubKey && pubKey.toString() }));
        return pubKey;
    } catch (err) {
        let error = {
            message: err.message,
        ...
        dnsCache.set(rr, JSON.stringify({ error }));
        throw err;
    }
};
