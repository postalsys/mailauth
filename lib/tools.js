...
const escapePropValue = (value, skipEscaping = false) => {
    ...
    return skipEscaping ? value : `"${value.replace(/["\\]/g, c => `\\${c}`)}"`;
};
    ...
    for (let ptype of ['policy', 'smtp', 'body', 'header']) {
        ...
        for (let prop of Object.keys(status[ptype])) {
            if (status[ptype][prop]) {
                parts.push(
                    `${ptype}.${prop}=${escapePropValue(status[ptype][prop], typeof status[ptype][prop] === 'object' && status[ptype][prop]?.skipEscaping)}`
                );
            }
        }
    }
...
function parseTagValueRecord(record, options = {}) {
    ...
    for (let part of parts) {
        ...
        if (normalizedKey in tags) {
            warnings.push(`Duplicate tag "${key}" - using last value`);
        }
...
// Helper function to validate specific record types
function validateRecordType(record, recordType) {
    const configs = {
        BIMI: {
            requiredTags: ['v', 'l', 'a'],
            allowedTags: ['v', 'l', 'a'],
    ...
    // Apply specific validators if available
